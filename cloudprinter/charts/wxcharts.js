...
function calCategoriesData(categories2, opts, config) {
    var result = {
        angle: 0,
        xAxisHeight: config.xAxisHeight
    };

    var _getXAxisPoints = getXAxisPoints(categories2, opts, config),
        eachSpacing = _getXAxisPoints.eachSpacing;

    // get max length of categories text


    var categoriesTextLenth = categories2.map(function (item) {
        return measureText(item);
    });
...
function getXAxisPoints(categories2, opts, config) {
    var yAxisTotalWidth = config.yAxisWidth + config.yAxisTitleWidth;
    var spacingValid = opts.width - 2 * config.padding - yAxisTotalWidth;
    var eachSpacing = spacingValid / categories2.length;

    var xAxisPoints = [];
    var startX = config.padding + yAxisTotalWidth;
    var endX = opts.width - config.padding;
    categories2.forEach(function (item, index) {
...
function drawRadarLabel(angleList, radius, centerPosition, opts, config, context) {
    ...
    angleList.forEach(function (angle, index) {
        ...
        if (util.approximatelyEqual(pos.x, 0)) {
            startX -= measureText(opts.categories2[index] || '') / 2;
        } else if (pos.x < 0) {
            startX -= measureText(opts.categorie2[index] || '');
        }
        context.fillText(opts.categories[index] || '', startX, startY + config.fontSize / 2);
        context.fillText(opts.categories2[index] || '', startX, startY + config.fontSize / 2 + 20);
    });
    context.stroke();
    context.closePath();
}
...
function drawColumnDataPoints(series, opts, config, context) {
    ...
    var _getXAxisPoints = getXAxisPoints(opts.categories2, opts, config),
...
function drawAreaDataPoints(series, opts, config, context) {
    ...
    var _getXAxisPoints2 = getXAxisPoints(opts.categories2, opts, config),
...
function drawLineDataPoints(series, opts, config, context) {
    ...
    var _getXAxisPoints3 = getXAxisPoints(opts.categories2, opts, config),
...
function drawXAxis(categories, categories2, opts, config, context) {
    var _getXAxisPoints4 = getXAxisPoints(categories2, opts, config),
    ...
    var maxXAxisListLength = Math.min(categories2.length, Math.ceil(validWidth / config.fontSize / 1.5));
    var ratio = Math.ceil(categories2.length / maxXAxisListLength);
    ...
    if (config._xAxisTextAngle_ === 0) {
        ...
        categories2.forEach(function (item, index) {
          var offset = eachSpacing / 2 - measureText(item) / 2;
          context.fillText(item, xAxisPoints[index] + offset, startY + config.fontSize + 15);
        });
        context.closePath();
        context.stroke();
    } else {
...
function drawRadarDataPoints(series, opts, config, context) {
    ...
    var radarOption = opts.extra.radar || {};
    var coordinateAngle = getRadarCoordinateSeries(opts.categories2.length);
    var centerPosition = {
        x: opts.width / 2,
        y: (opts.height - config.legendHeight) / 2
    };

    var radius = Math.min(centerPosition.x - (getMaxTextListLength(opts.categories2) + config.radarLabelTextMargin), centerPosition.y - config.radarLabelTextMargin);
...
function drawCharts(type, opts, config, context) {
    ...
    var categories2 = opts.categories2;
    ...
    if (categories2 && categories2.length) {
        var _calCategoriesData = calCategoriesData(categories2, opts, config),
    ...
    switch (type) {
        ...
        case 'column':
            this.animationInstance = new Animation({
                ...
                onProcess: function onProcess(process) {
                    drawYAxis(series, opts, config, context);
                    drawXAxis(categories, categories2, opts, config, context);
...
Charts.prototype.updateData = function () {
    ...
    this.opts.categories2 = data.categories2 || this.opts.categories2;
...
Charts.prototype.getCurrentDataIndex = function (e) {
    if (e.touches && e.touches.length) {
        ...
        if (this.opts.type === 'pie' || this.opts.type === 'ring') {
            ...
            return findRadarChartCurrentIndex({ x: x, y: y }, this.chartData.radarData, this.opts.categories2.length);
        } else {
